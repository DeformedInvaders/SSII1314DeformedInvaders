...
import com.example.utils.BooleanArray;
import com.example.utils.FloatArray;
import com.example.utils.IntArray;
...
public class DelaunayTriangulator {
	...
	public void trim (ShortArray triangles, FloatArray points, FloatArray hull, int offset, int count) {
		...
		while(j < triangles.size) {
			short p1 = triangles.get(j);
			short p2 = triangles.get(j + 1);
			short p3 = triangles.get(j + 2);
			
			float p1x = points.get(2*p1);		float p1y = points.get(2*p1 + 1);
			float p2x = points.get(2*p2);		float p2y = points.get(2*p2 + 1);
			float p3x = points.get(2*p3);		float p3y = points.get(2*p3 + 1);
			
			GeometryUtils.triangleCentroid(p1x, p1y, p2x, p2y, p3x, p3y, centroid);
			
			if (Intersector.isPointInPolygon(hull.items, offset, count, centroid.x, centroid.y)) {	
				trianglesTmp.add(p1);
				trianglesTmp.add(p2);
				trianglesTmp.add(p3);
			}
	...
	public void mesh (ShortArray triangles, ShortArray mesh, FloatArray points, int count, float maxArea) {
		...
		int p4 = points.size / 2;
		boolean areaMin = true;
		
		while(trianglesIn.size/3 < count && areaMin) {
			
			int j = 0;
			areaMin = false;
			
			while(j < trianglesIn.size) {
				short p1 = trianglesIn.get(j);
				short p2 = trianglesIn.get(j + 1);
				short p3 = trianglesIn.get(j + 2);
				...
				vertices.add(p1x);	vertices.add(p1y);
				vertices.add(p2x);	vertices.add(p2y);
				vertices.add(p3x);	vertices.add(p3y);
				...
				if(area > maxArea) {
					...
					trianglesOut.add(p1);	trianglesOut.add(p4);	trianglesOut.add(p6);
					trianglesOut.add(p4);	trianglesOut.add(p2);	trianglesOut.add(p5);
					trianglesOut.add(p5);	trianglesOut.add(p3);	trianglesOut.add(p6);
					trianglesOut.add(p4);	trianglesOut.add(p5);	trianglesOut.add(p6);
					
					points.add(p4x);	points.add(p4y);
					points.add(p5x);	points.add(p5y);
					points.add(p6x);	points.add(p6y);
			...
			trianglesIn.clear();
			trianglesIn.addAll(trianglesOut);
			trianglesOut.clear();
		}	
		
		mesh.addAll(trianglesIn);
	}
}