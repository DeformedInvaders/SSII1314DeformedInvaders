...
/**
 * A simple quaternion class. See <a
 * href="http://en.wikipedia.org/wiki/Quaternion"
 * >http://en.wikipedia.org/wiki/Quaternion</a> for more information.
 * 
...
public class Quaternion implements Serializable {
	...
	public Quaternion setFromCross(final Vector3 v1, final Vector3 v2) {
		...
		return setFromAxis(v1.y * v2.z - v1.z * v2.y,
				v1.z * v2.x - v1.x * v2.z, v1.x * v2.y - v1.y * v2.x, angle);
	}
	...
	public Quaternion setFromCross(final float x1, final float y1,
		...
		final float dot = MathUtils.clamp(Vector3.dot(x1, y1, z1, x2, y2, z2),
				-1f, 1f);
		final float angle = (float) Math.acos(dot) * MathUtils.radiansToDegrees;
		return setFromAxis(y1 * z2 - z1 * y2, z1 * x2 - x1 * z2, x1 * y2 - y1
				* x2, angle);
	}
	...
	public float getAxisAngle(Vector3 axis) {
		...
		double s = Math.sqrt(1 - this.w * this.w); // assuming quaternion
													...
													// positive.
		if (s < NORMALIZATION_TOLERANCE) { // test to avoid divide by zero, s is
