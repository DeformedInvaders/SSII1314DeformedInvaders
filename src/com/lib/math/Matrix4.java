 ...
 * major</a> 4 by 4 matrix. Like the {@link Vector3} class it allows the chaining
...
public class Matrix4 implements Serializable {
	...
	public Matrix4 set(Vector3 position, Quaternion orientation, Vector3 scale) {
		...
		final float wx = orientation.w * xs, wy = orientation.w * ys, wz = orientation.w
				* zs;
		final float xx = orientation.x * xs, xy = orientation.x * ys, xz = orientation.x
				* zs;
		final float yy = orientation.y * ys, yz = orientation.y * zs, zz = orientation.z
				* zs;
	...
	public Vector3 getScale(Vector3 scale) {
		...
		scale.y = (float) Math.sqrt(val[M10] * val[M10] + val[M11] * val[M11]
				+ val[M12] * val[M12]);
		scale.z = (float) Math.sqrt(val[M20] * val[M20] + val[M21] * val[M21]
				+ val[M22] * val[M22]);
		return scale;
	}
	 ...
	 */
	public static native void mulVec(float[] mat, float[] vecs, int offset,
	 ...
	 *            the stride between vectors in floats
	 */
	public static native void rot(float[] mat, float[] vecs, int offset,
			int numVecs, int stride) /*-{ }-*/; /*
												 * float* vecPtr = vecs +
												 * offset; for(int i = 0; i <
												 * numVecs; i++) {
